...
class BuildInputsTest(unittest.TestCase):
  ...
  def test_optional_overrides_omitted_by_default_but_included_when_set(self):
    inputs = gi.build_inputs(self._sample(), self.site_config, self.repo_root, "off")
    for field in gi.common.HIFI_ASSEMBLY_OPTIONAL_BOOLEAN_FIELDS + gi.common.HIFI_ASSEMBLY_OPTIONAL_INT_FIELDS:
      self.assertNotIn(f"EndToEndAssembly.{field}", inputs)

    inputs = gi.build_inputs(
      self._sample(assemble_mitogenome=False, min_hom_cov=5), self.site_config, self.repo_root, "off",
    )
    self.assertIs(inputs["EndToEndAssembly.assemble_mitogenome"], False)
    self.assertEqual(inputs["EndToEndAssembly.min_hom_cov"], 5)
    self.assertNotIn("EndToEndAssembly.override_hom_cov", inputs)
